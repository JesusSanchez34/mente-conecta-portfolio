...
import { loginApiEnvejecimiento } from '../../../api/user';
import { useAuth } from '../../../hooks';
import { useNavigate } from 'react-router-dom';
import { useSettings } from '../../../context/SettingsContext';
import './FormLoginEnvejecimiento.css';
...
export function FormLoginEnvejecimiento(props) {
    const { onBack } = props;
    const { login } = useAuth();
    ...
    const [rememberMe, setRememberMe] = useState(() => {
        return localStorage.getItem('env_remember_username') ? true : false;
    });
    const [selectedCountry, setSelectedCountry] = useState(() => {
        const savedCountry = localStorage.getItem('env_remember_country');
        if (savedCountry) {
            return countriesData.find(c => c.code === savedCountry) || countriesData.find(c => c.code === 'mx');
        }
        return countriesData.find(c => c.code === 'mx');
    });
    ...
    const formik = useFormik({
        initialValues: {
            username: (() => { const v = localStorage.getItem('env_remember_username') || ''; if (v.length >= 8) return v; localStorage.removeItem('env_remember_username'); return ''; })(),
            password: localStorage.getItem('env_remember_password') || "",
        },
        validationSchema: Yup.object({
            username: Yup.string()
                .matches(/^[0-9]+$/, language === 'en' ? "Phone number must contain only digits" : "El número de teléfono debe contener solo dígitos")
                .min(8, language === 'en' ? "Number is too short" : "El número es demasiado corto")
                .max(15, language === 'en' ? "Number is too long" : "El número es demasiado largo")
                .required(language === 'en' ? "Enter your phone number" : "Ingrese su número de teléfono"),
            password: Yup.string().required(language === 'en' ? "Enter your password" : "Ingrese su contraseña"),
        }),
        ...
        onSubmit: async (formvalue) => {
            try {
                ...
                const celular_paciente = `${selectedCountry.dial_code}${formvalue.username}`;
                const payload = { celular_paciente, password: formvalue.password };

                const response = await loginApiEnvejecimiento(payload);
                const { access } = response;

                if (rememberMe) {
                    localStorage.setItem('env_remember_username', formvalue.username);
                    localStorage.setItem('env_remember_password', formvalue.password);
                    localStorage.setItem('env_remember_country', selectedCountry.code);
                } else {
                    localStorage.removeItem('env_remember_username');
                    localStorage.removeItem('env_remember_password');
                    localStorage.removeItem('env_remember_country');
                }

                await login(access, 6);
                navigate('/quien-lo-realiza');
            } catch (error) {
                console.error(`[FormLoginEnvejecimiento] Error en login:`, error);
                if (error.status === 401) {
                    const msg = language === 'en' ? "Incorrect password or unregistered number." : "Contraseña incorrecta o número no registrado.";
                    setError(msg); toast.error(msg);
                } else if (error.status === 404) {
                    const msg = language === 'en' ? "This number is not registered." : "El número ingresado no está registrado.";
                    setError(msg); toast.warning(msg);
                } else if (error.message?.includes("no tiene acceso")) {
                    const msg = language === 'en' ? "Your account has no access to this platform." : "Tu cuenta no tiene acceso a esta plataforma.";
                    setError(msg); toast.error(msg);
                } else {
                    const msg = error.message || (language === 'en' ? "Connection error. Check your internet." : "Error de conexión. Verifica tu internet.");
                    setError(msg); toast.error(msg);
                }
            } finally {
                setIsLoading(false);
    ...
    const phoneLength = formik.values.username.length;
    const phoneIncomplete = phoneLength > 0 && phoneLength < selectedCountry.maxLength;

    useEffect(() => {
        const stored = localStorage.getItem('env_remember_username');
    ...
    return (
        <div className="env-login-bg">
            {/* Language toggle — top right */}
            <button className="env-lang-toggle" type="button" onClick={toggleLanguage} title={language === 'en' ? 'Cambiar a Español' : 'Switch to English'}>
                <span className="env-lang-flag">{getFlagEmoji(language === 'en' ? 'us' : 'mx')}</span>
                <FiGlobe className="env-lang-globe" />
            </button>

            <div className="env-login-wrapper">
                <div className="env-main-card">

                    <div className="env-header-section">
                        <div className="env-logo-container">
                            <img src={logoColor} alt="logo" className="env-logo-img" />
                        </div>
                        <h2>{language === 'en' ? 'Welcome!' : '¡Bienvenido/a!'}</h2>
                        <p>{language === 'en' ? 'Enter your phone number and password to continue.' : 'Ingresa tu número de teléfono y contraseña para continuar.'}</p>
                    </div>

                    {error && (
                        <div className="env-error-banner" role="alert">
                            <div className="env-error-content">
                                <strong>Error:</strong> {error}
                            </div>
                            <button type="button" className="env-error-close-btn" onClick={() => setError(null)} aria-label="Cerrar">&times;</button>
                        </div>
                    )}

                    <form onSubmit={formik.handleSubmit} className="env-form">

                        {/* Phone field */}
                        <div className="env-input-group">
                            <span className="env-floating-label">{language === 'en' ? 'Mobile phone' : 'Teléfono móvil'}</span>
                            <div className="env-input-wrapper" style={{ borderColor: phoneIncomplete ? '#e53e3e' : undefined }}>
                                <div ref={dropdownRef} className="env-phone-prefix" onClick={() => setShowCountryDropdown(!showCountryDropdown)}>
                                    <span className="env-flag-emoji">{getFlagEmoji(selectedCountry.code)}</span>
                                    <span className="arrow">▼</span>
                                    <span className="prefix-code">{selectedCountry.dial_code}</span>

                                    {showCountryDropdown && (
                                        <div className="env-country-dropdown" onClick={(e) => e.stopPropagation()}>
                                            <div className="env-country-search-wrapper">
                                                <input
                                                    ref={searchInputRef}
                                                    type="text"
                                                    className="env-country-search"
                                            ...
                                            <div className="env-country-list">
                                                {filteredCountries.length === 0 ? (
                                                    <div className="env-country-no-results">
                                                        {language === 'en' ? 'No results' : 'Sin resultados'}
                                                    </div>
                                                ) : filteredCountries.map((country) => (
                                                    <div
                                                        key={country.code}
                                                        className={`env-country-item${selectedCountry.code === country.code ? ' selected' : ''}`}
                                                        onClick={() => {
                                                    ...
                                                    >
                                                        <span className="env-flag-emoji">{getFlagEmoji(country.code)}</span>
                                                        <span className="env-country-name">{getCountryName(country)}</span>
                                                        <span className="env-country-dial">{country.dial_code}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                ...
                                <div className="env-phone-divider"></div>
                                <input
                                    type="tel"
                                    name="username"
                                    placeholder={language === 'en' ? 'Mobile phone' : 'Teléfono móvil'}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                                <span style={{ color: '#e53e3e', fontSize: '0.72rem', paddingLeft: '4px' }}>
                                    {phoneIncomplete
                                        ? (language === 'en' ? 'Invalid or incomplete phone number' : 'Número de teléfono inválido o incompleto')
                                        : (formik.errors.username && formik.touched.username ? formik.errors.username : '')
                                    }
                                </span>
                                <span className={`env-char-counter${phoneIncomplete ? ' env-char-counter-error' : ''}`}>
                        ...
                        {/* Password field */}
                        <div className="env-input-group" style={{ marginTop: '0.2rem' }}>
                            <div className="env-input-wrapper">
                                ...
                                <button
                                    type="button"
                                    className="env-toggle-password"
                                    onClick={() => setShowPassword(!showPassword)}
                                    title={showPassword ? (language === 'en' ? 'Hide password' : 'Ocultar contraseña') : (language === 'en' ? 'Show password' : 'Mostrar contraseña')}
                                >
                                    {showPassword ? <FaEyeSlash /> : <FaEye />}
                                </button>
                            </div>
                            {formik.errors.password && formik.touched.password && (
                                <span style={{ color: '#e53e3e', fontSize: '0.72rem', marginTop: '4px', paddingLeft: '4px' }}>
                        ...
                        <div className="env-options-row">
                            <label className="env-checkbox-label">
                                <input
                                    type="checkbox"
                                    checked={rememberMe}
                            ...
                            <button type="button" className="env-forgot-link" onClick={handleForgotPassword}>
                                {language === 'en' ? 'Forgot password?' : '¿Olvidaste tu contraseña?'}
                            </button>
                        </div>

                        <button type="submit" className="env-btn-siguiente" disabled={isLoading}>
                            {language === 'en' ? 'Next' : 'Siguiente'}
                        </button>
                        <button type="button" className="env-btn-regresar" onClick={onBack} disabled={isLoading}>
            ...
            {isLoading && (
                <div className="env-loading-overlay">
                    <div className="env-loading-card">
                        <h3 className="env-loading-title">{language === 'en' ? 'Signing in...' : 'Iniciando sesión ...'}</h3>
                        <Spinner animation="border" style={{ width: '3.5rem', height: '3.5rem', borderWidth: '0.25em', color: '#2196F3' }} />
                    </div>
                </div>
            )}
        </div>
