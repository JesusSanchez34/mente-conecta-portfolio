...
export const ReportePdfDocumentNX = ({ reporte, t }) => {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          ...
          {reporte?.resultados?.map((res, index) => {
            const cuestionarioLimpio = (res.cuestionario || '').replace(/\(\d+\)$/, '').trim();

            return (
              <View style={styles.tableRow} key={index}>
                <Text style={[styles.tableCell, styles.col1]}>
                  {t(`pdf.results.${cuestionarioLimpio}`, cuestionarioLimpio)}
                </Text>
                <Text style={[styles.tableCell, styles.col2]}>{res.nota ?? ''}</Text>
                <Text style={[styles.tableCell, styles.col3]}>
                  {t(`pdf.results.${res.interpretacion}`, res.interpretacion || '')}
                </Text>
              </View>
            );
          })}
