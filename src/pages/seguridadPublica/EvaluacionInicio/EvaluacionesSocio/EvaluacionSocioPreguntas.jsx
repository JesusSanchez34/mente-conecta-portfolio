...
export function EvaluacionSocioPreguntas() {
    const { t, i18n } = useTranslation();
    const tituloSeccion = "Sociodemográficos";
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                // 1 is Sociodemografica section
                const cuestionarios = await getCuestionariosPorSeccionApi(token, 1);
    ...
    const handleOptionSelect = (pregunta, respuesta, textVal = '') => {
        ...
        if (isMulti) {
            ...
            if (index > -1) {
                ...
                currentTexts.push(textVal || respuesta.respuesta);
            }
            ...
            newRespuestasUser[pregunta.id] = {
                ...
                respuestasUser: textVal || respuesta.respuesta,
                calificacion: respuesta.ponderacion || 0,
    ...
    const executeSubmit = async () => {
        ...
        try {
            ...
            if (payload.estatus_finalizado === 1 && cuestionario.genera_resultados === 1) {
                ...
                navigate('/seguridad/evaluacion-socio');
            }
        ...
        return (
            <div className="evaluacion-layout">
                <div className="evaluacion-question-card text-center" style={{ padding: '50px' }}>
                    ...
                    <button className="btn-regresar" onClick={() => navigate('/seguridad/evaluacion-socio')}>
                        {t('survey.return_to_eval', 'Volver a Sociodemográficos')}
                    </button>
                </div>
            </div>
        );
        ...
        <div className="evaluacion-layout">
            <div className="evaluacion-question-card">
                <div className="cuestionario-header-row">
                    ...
                    {(i18n.language?.startsWith('en') ? (cuestionario?.instructions || t('evaluations.socio_sub_desc')) : cuestionario?.instrucciones) && (
                        <button
                ...
                <h2 style={{ textAlign: 'left', marginBottom: '8px' }}>{i18n.language?.startsWith('en') ? (cuestionario?.title || t('evaluations.socio_sub_modal_title', 'Sociodemographics')) : cuestionario?.titulo}</h2>
                            ...
                            <div key={pregunta.id} className="pregunta-card-item">
                                <div className="pregunta-title-row">
                                    <h4 className="pregunta-title">
                                        {questionIndex}. {i18n.language?.startsWith('en') ? (pregunta.question || pregunta.pregunta) : pregunta.pregunta}
                                    </h4>
                                    {((pregunta.explicacionPregunta || pregunta.explicacion_pregunta)) && (
                                        <span 
                                            className="pregunta-help-icon"
                                            data-tooltip={i18n.language?.startsWith('en') ? (pregunta.explanation_question || pregunta.explicacionPregunta || pregunta.explicacion_pregunta) : (pregunta.explicacionPregunta || pregunta.explicacion_pregunta)}
                                            onClick={() => alert(i18n.language?.startsWith('en') ? (pregunta.explanation_question || pregunta.explicacionPregunta || pregunta.explicacion_pregunta) : (pregunta.explicacionPregunta || pregunta.explicacion_pregunta))}
                                        >
                ...
                <div className="cuestionario-footer-buttons">
                    <div className="pagination-arrows">
                        {currentPage > 0 && (
                            <button className="btn-paginator" onClick={handlePrevPage}>
                                {t('survey.prev_btn', 'Anterior')}
                            </button>
                        )}
                        {currentPage < totalPages - 1 && (
                            <button className="btn-paginator" onClick={handleNextPage}>
                                {t('survey.next_btn', 'Siguiente')}
                            </button>
                        )}
                    </div>
                    ...
                    <div className="mental-modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="mental-modal-body">
                            ...
                            <p className="mental-modal-description">
                                {i18n.language?.startsWith('en') ? (cuestionario?.instructions || cuestionario?.instrucciones) : cuestionario?.instrucciones}
                            </p>
                            <button className="mental-modal-btn" onClick={() => setShowInstructionsModal(false)}>
                                {t('survey.close_btn', 'Cerrar')}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="mental-modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="mental-modal-body">
                            ...
                            <div className="exit-modal-footer">
                                ...
                                <button className="exit-modal-btn-exit" onClick={() => navigate('/seguridad/evaluacion-socio')}>
